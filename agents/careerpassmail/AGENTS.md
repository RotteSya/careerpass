...
## 噪音过滤（必须排除，isJobRelated=false）

- マイナビ / リクナビ / OfferBox / ONE CAREER / 等就职平台发出的「站内信通知 / メッセージが届きました / 新着メッセージのお知らせ / お知らせが届いています」这种**只是提示用户去站内查看**的壳邮件——它们本身没有具体公司、时间、行动，不构成事件信号。
  - 判定信号：发件人是平台（mynavi/rikunabi/offerbox 等域名）、正文核心是「ログインして確認」「マイページでご確認ください」、不含具体公司名/日时/地点/可执行行动。
  - 正确做法：isJobRelated=false，reason 注明「平台站内信壳通知，无具体事件信号」。
- 例外：如果同一封平台邮件正文中**直接**给出了具体公司名 + 面接/説明会/締切的日期时间或明确动作，则按真实事件抽取，不要丢弃。

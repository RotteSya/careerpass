...
## 自己分析`;
    ...
    reconCompany: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        // Generate company deep report using LLM (Firecrawl/Tavily integration via Python agent)
        const systemPrompt = `あなたは企業リサーチの専門家です。以下の企業について、日本の就活生向けの詳細な企業情報レポートを作成してください。

レポート形式（${input.companyName}_Recon_Report.md）：

# 【基本情報と中期戦略】
企業の主要事業、技術スタック、市場地位、最近の戦略的重点
...
面接で使える効果的な逆質問3〜5個`;
        ...
        const response = await invokeLLM({
          messages: [
            ...
            {
              role: "user",
              content: `${input.companyName}についての詳細な企業情報レポートを作成してください。`,
            },
          ],
        });
        ...
        await saveAgentMemory({
          ...
          metadata: { companyName: input.companyName, jobApplicationId: input.jobApplicationId },
        });

        return { report: reportContent, companyName: input.companyName };
      }),

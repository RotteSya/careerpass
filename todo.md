...
## v2.7 根本修复 session 认证（用户反馈 cookie 方案失效）
- [x] 改用 localStorage + Authorization header 方案替代 HTTP-only cookie，彻底绕过 SameSite/Secure cookie 限制
- [x] 更新 Login.tsx / EmailVerified.tsx / useAuth.ts 使用新认证方案
- [x] 更新 server 端 sdk.ts 同时支持 cookie 和 Authorization header 读取 session token
